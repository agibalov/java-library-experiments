...
public class VariableDefinitionStatementTest {
	private final String expression;
	private final ExpressionParser parser;	
	private final ParseResultExpectation parseResultExpectation;
	
	public VariableDefinitionStatementTest(			
			String expression,
	...
	public static Collection<Object[]> makeTestData() {
		...
		parameters.add(new Object[] { " int x = 1 ", parsePureStatement(), result(               
                isStatement(isVariableDefinition(
                        hasTypeReference(isNamedTypeReference(withTypeName("int"))),
                        hasVariableName("x"),
                        hasInitializerExpression(isIntLiteralWithValueOf("1"))))
                ) });
